...
function tagField(id, label, hint, tags, onAdd, onRemove) {
	return import_react.createElement("div", { className: "dtt-field" }, import_react.createElement("div", { className: "dtt-fieldHead" }, import_react.createElement("label", {
		className: "dtt-label",
		htmlFor: id
	}, label)), hint ? import_react.createElement("span", { className: "dtt-hint" }, hint) : null, import_react.createElement("div", { className: "dtt-tags" }, tags.map((tag) => import_react.createElement("span", {
		key: tag,
		className: "dtt-tag",
		onClick: () => onRemove(tag)
	}, `× ${tag}`)), import_react.createElement("input", {
...
function useActiveLocale(ctx) {
	...
	return import_react.useSyncExternalStore(import_react.useMemo(() => (cb) => {
...
function TinyToolSettingsForm({ t, ctx }) {
	...
	const [saved, setSaved] = import_react.useState(null);
	const [saving, setSaving] = import_react.useState(false);
	const [err, setErr] = import_react.useState("");
	const scope = import_react.useMemo(() => {
		const svc = getService(ctx, "settingsScope");
		return svc && typeof svc.bind === "function" ? svc.bind({ namespace: NS }) : void 0;
	}, [ctx]);
	const snapshot = import_react.useSyncExternalStore(import_react.useMemo(() => (cb) => scope ? scope.subscribe(cb) : () => {}, [scope]), import_react.useCallback(() => scope ? scope.getSnapshot() : { status: "ready" }, [scope]), import_react.useCallback(() => ({ status: "loading" }), []));
	const status = snapshot?.status || "ready";
	const writable = snapshot?.writable !== void 0 ? snapshot.writable : true;
	import_react.useEffect(() => {
		if (status === "unavailable") return;
		let alive = true;
		fetch("/dsh-tiny-tool/config", { cache: "no-store" }).then((res) => res.json()).then((data) => {
			if (!alive) return;
	...
	const dirty = !!(draft && saved && (JSON.stringify(draft.exemptTools) !== JSON.stringify(saved.exemptTools) || JSON.stringify(draft.exemptPrefixes) !== JSON.stringify(saved.exemptPrefixes)));
	...
	if (status === "loading" || !draft && !err && status !== "unavailable") return import_react.createElement("p", { className: "dtt-failed" }, t("loading"));
	if (status === "unavailable") return import_react.createElement("p", { className: "dtt-failed" }, t("settingsUnavailable"));
	return import_react.createElement(import_react.Fragment, null, import_react.createElement("p", { className: "dtt-hint" }, t("intro")), tagField("dtt-exempt-tools", t("exemptTools"), t("exemptToolsHint"), draft?.exemptTools ?? [], (tag) => setField("exemptTools", [...draft?.exemptTools ?? [], tag]), (tag) => setField("exemptTools", (draft?.exemptTools ?? []).filter((x) => x !== tag))), tagField("dtt-exempt-prefixes", t("exemptPrefixes"), t("exemptPrefixesHint"), draft?.exemptPrefixes ?? [], (tag) => setField("exemptPrefixes", [...draft?.exemptPrefixes ?? [], tag]), (tag) => setField("exemptPrefixes", (draft?.exemptPrefixes ?? []).filter((p) => p !== tag))), import_react.createElement("div", { className: "dtt-foot" }, err ? import_react.createElement("p", { className: "dtt-failed" }, err || t("saveFailed")) : null, import_react.createElement("button", {
		type: "button",
	...
	}, t("discard")), import_react.createElement("button", {
		type: "button",
		className: "dtt-save",
		disabled: blocked,
		onClick: save
...
function TinyToolCard({ t, open, onOpen, children }) {
	return import_react.createElement("li", { className: "dtt-card" + (open ? " dtt-cardOpen" : "") }, import_react.createElement("button", {
		type: "button",
		className: "dtt-head",
		"aria-expanded": open,
		"aria-label": (open ? t("collapse") : t("expand")) + ": " + t("title"),
		onClick: () => onOpen((v) => !v)
	}, import_react.createElement("span", { className: "dtt-headText" }, import_react.createElement("span", { className: "dtt-title" }, t("title")), import_react.createElement("span", { className: "dtt-sub" }, t("description"))), import_react.createElement("span", { className: "dtt-chevron" + (open ? " dtt-chevronOpen" : "") }, "▾")), import_react.createElement("div", {
		className: "dtt-body",
		hidden: !open,
...
function TinyToolPluginCard({ ctx }) {
	...
	return import_react.createElement(TinyToolCard, {
		t,
		open,
		onOpen: setOpen,
		children: import_react.createElement(TinyToolSettingsForm, {
			t,
			ctx
		})
	});
...
function apply(ctx) {
	...
	if (slotsSvc && typeof slotsSvc.inject === "function") try {
		slotsSvc.inject("settings.plugin.item", () => slotsSvc.register({
			name: "settings.plugin.item",
			key: NS,
			locale: NS,
			inject: () => ({ ctx })
		}, (props) => import_react.createElement(TinyToolPluginCard, Object.assign({}, props, { ctx }))));
	} catch (err) {
...
//#endregion
export { _moduleExports, apply, inject };
